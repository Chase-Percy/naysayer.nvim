local path_sep = require("naysayer").path_sep
...
function M.compile(flavour)
	local theme = require("naysayer.lib.mapper").apply(flavour)
	local lines = {
		string.format(
			[[
...
local h = vim.api.nvim_set_hl]],
	...
	if vim.g.naysayer_debug then -- Debugging purpose
	...
	if not f then
		local err_path = (path_sep == "/" and "/tmp" or os.getenv "TMP") .. "/naysayer_error.lua"
		print(string.format(
			[[Naysayer (error): Most likely some mistake made in your naysayer config
