...
function M.get()
	return {
		...
		nCursor = { fg = C.darkest_green, bg = C.light_green }, -- character under the cursor
		...
		Directory = { fg = C.monokai_blue }, -- directory names (and other special names in listings)
		...
		ErrorMsg = { fg = C.monokai_red, style = { "bold", "italic" } }, -- error messages on the command line
		VertSplit = { fg = O.transparent_background and C.dark_green or C.brown }, -- the column separating vertically split windows
		Folded = { fg = C.monokai_blue, bg = O.transparent_background and C.none or C.dark_green }, -- line used for closed folds
		...
		MoreMsg = { fg = C.monokai_blue }, -- |more-prompt|
		...
		NormalFloat = {
			fg = C.white,
			bg = (O.transparent_background and vim.o.winblend == 0) and C.none or C.darker_green,
		...
		Question = { fg = C.monokai_blue }, -- |hit-enter| prompt and yes/no questions
		...
		CurSearch = { bg = C.monokai_red, fg = C.darkest_green }, -- 'cursearch' highlighting: highlights the current search you're on differently
		...
		SpellBad = { sp = C.monokai_red, style = { "undercurl" } }, -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
		SpellCap = { sp = C.monokai_yellow, style = { "undercurl" } }, -- Word that should start with a capital. |spell| Combined with the highlighting used otherwise.
		SpellLocal = { sp = C.monokai_blue, style = { "undercurl" } }, -- Word that is recognized by the spellchecker as one that is used in another region. |spell| Combined with the highlighting used otherwise.
		...
		StatusLine = { fg = C.brown, bg = O.transparent_background and C.none or C.brown }, -- status line of current window
		StatusLineNC = { fg = C.brown, bg = O.transparent_background and C.none or C.brown }, -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
		...
		Title = { fg = C.monokai_blue, style = { "bold" } }, -- titles for output from ":set all", ":autocmd" etc.
		...
		WarningMsg = { fg = C.monokai_yellow }, -- warning messages
