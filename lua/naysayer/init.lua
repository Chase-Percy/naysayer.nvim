...
if is_vim then require "naysayer.lib.vim" end
...
function M.compile()
	...
	for flavour, _ in pairs(M.flavours) do
		M.flavour = flavour
		require("naysayer.lib." .. (is_vim and "vim." or "") .. "compiler").compile(flavour)
	end
...
local function get_flavour(default)
	...
	if flavour and not M.flavours[flavour] then
		vim.notify(
			string.format(
				"Naysayer (error): Invalid flavour '%s', flavour must be 'latte', 'frappe', 'macchiato' or 'mocha'",
				flavour
			),
	...
	return flavour or M.options.flavour or vim.g.naysayer_flavour or M.options.background[vim.o.background]
end
...
---@type fun(user_conf: NaysayerOptions?)
function M.setup(user_conf)
	...
	local hash = require("naysayer.lib.hashing").hash(user_conf)
...
vim.api.nvim_create_user_command(
	"Naysayer",
	function(inp) vim.api.nvim_command("colorscheme naysayer-" .. get_flavour(inp.args)) end,
	{
		nargs = 1,
...
vim.api.nvim_create_user_command("NaysayerCompile", function()
	for name, _ in pairs(package.loaded) do
		if name:match "^naysayer." then package.loaded[name] = nil end
	end
	M.compile()
	vim.notify("Naysayer (info): compiled cache!", vim.log.levels.INFO)
	vim.cmd.colorscheme "naysayer"
end, {})

if vim.g.naysayer_debug then
	vim.api.nvim_create_autocmd("BufWritePost", {
		pattern = "*/naysayer/*",
		callback = function()
			vim.schedule(function() vim.cmd "NaysayerCompile" end)
		end,
	})
end
